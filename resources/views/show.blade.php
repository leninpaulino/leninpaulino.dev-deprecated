...
<div class="dropdown">
    <a href="#" aria-label="Settings" class="nav-link px-3" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>
        <svg xmlns="http://www.w3.org/2000/svg" width="25" viewBox="0 0 24 24" class="icon-cog">
            ...
            <circle cx="12" cy="12" r="2" class="primary" /></svg>
    </a>
    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdown">
        <a href="{{ url(sprintf('%s/posts/%s/edit', config('canvas.path'), $data['post']->id)) }}" class="dropdown-item">{{ __('canvas::blog.buttons.edit') }}</a>

        <a href="{{ url(sprintf('%s/stats/%s', config('canvas.path'), $data['post']->id)) }}" class="dropdown-item">{{ __('canvas::blog.buttons.stats') }}</a>
    </div>
</div>
@endif
@endsection
    ...
    <div class="row justify-content-md-center">
        <div class="col col-lg-8">
            ...
            <div class="media py-1">
                <img src="{{ sprintf('%s%s%s', 'https://secure.gravatar.com/avatar/', md5(strtolower(trim($data['author']->email))), '?s=200') }}" class="mr-3 rounded-circle shadow-sm" style="width: 50px" alt="{{ $data['author']->name }}">
            ...
            @isset($data['post']->featured_image)
            <img src="{{ $data['post']->featured_image }}" class="w-100 pt-4" @isset($data['post']->featured_image_caption) alt="{{ $data['post']->featured_image_caption }}"
...
<div class="read-more mt-5 container-fluid">
    <div class="row">
        @isset($data['next'])
        <div class="col-lg bg-light text-center px-lg-5 py-5" @isset($data['next']->featured_image) style="background: linear-gradient(rgba(0, 0, 0, 0.8),rgba(0, 0, 0, 0.8)),url({{ $data['next']->featured_image }}); background-size: cover" @endisset>
            <a href="{{ route('blog.post', $data['next']->slug) }}" class="btn btn-sm text-decoration-none @isset($data['next']->featured_image) btn-outline-light @else btn-outline-secondary @endisset text-uppercase font-weight-bold mt-3">
        ...
        @endisset
        @isset($data['random'])
        <div class="col-lg bg-light text-center px-lg-5 py-5" @isset($data['random']->featured_image) style="background: linear-gradient(rgba(0, 0, 0, 0.8),rgba(0, 0, 0, 0.8)),url({{ $data['random']->featured_image }}); background-size: cover" @endisset>
